...
async function kalshiRequest(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      lastError = new Error(
        `Kalshi API ${method} ${path} → ${response.status}: ${text}`
      );
      ...
      if (!isNetwork || attempt === MAX_RETRIES) throw lastError;
      await sleep(RETRY_DELAY_MS * (attempt + 1));
    }
  }
...
export async function getBtc15mMarkets(
  ...
  return (data.markets ?? []).map((m: any) => {
    ...
    const last_price = parseCents(
      m.last_price_dollars,
      m.last_price ?? yes_bid
    );
...
export async function getOpenPositions(
  ...
  const data = await kalshiRequest(
    "GET",
    "/portfolio/positions?settlement_status=unsettled&limit=100",
    apiKeyId,
    privateKeyPem,
    base
  );
...
export async function getSettledPositions(
  ...
  try {
    const data = await kalshiRequest(
      "GET",
      "/portfolio/positions?settlement_status=settled&limit=20",
      apiKeyId,
      privateKeyPem,
      base
    );
...
export async function getOrderFills(
  ...
  try {
    const data = await kalshiRequest(
      "GET",
      `/portfolio/fills?order_id=${orderId}&limit=10`,
      apiKeyId,
      privateKeyPem,
      base
    );
